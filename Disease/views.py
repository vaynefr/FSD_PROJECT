...
def covid1(request):
    ...
    if request.method == "POST":
        ...
        shortness_of_breath = (
            True if request.POST.get("shortness_of_breath") == "True" else False
        )
        head_ache = True if request.POST.get("head_ache") == "True" else False
        corona_result = request.POST["corona_result"]
        age_60_and_above = (
            True if request.POST.get("age_60_and_above") == "True" else False
        )
...
def liver(request):
    ...
    if request.method == "POST":
        ...
        Alkphos_Alkaline_Phosphotase = float(
            request.POST["Alkphos_Alkaline_Phosphotase"]
        )
